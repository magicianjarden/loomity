...
export default function DocumentsPage() {
  ...
  const groupDocuments = (docs: Document[]): DocumentGroup[] => {
    ...
    const now = new Date('2024-12-15T19:17:05-05:00');

    // First add pinned documents if any exist
    const pinnedDocs = docs.filter(doc => doc.is_pinned);
    ...
    // Then add favorite documents if any exist
    const favoriteDocs = docs.filter(doc => doc.is_favorite && !doc.is_pinned);
    ...
    // Group remaining documents by time
    const remainingDocs = docs.filter(doc => !doc.is_pinned && !doc.is_favorite);
    
    const todayDocs = remainingDocs.filter(doc => 
      isToday(new Date(doc.updated_at))
    );
    if (todayDocs.length > 0) {
      groups.push({
        title: 'Today',
        documents: todayDocs.sort((a, b) => 
          new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime()
        )
      });
    }

    const yesterdayDocs = remainingDocs.filter(doc => 
      isYesterday(new Date(doc.updated_at))
    );
    if (yesterdayDocs.length > 0) {
      groups.push({
        title: 'Yesterday',
        documents: yesterdayDocs.sort((a, b) => 
          new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime()
        )
      });
    }

    const thisWeekDocs = remainingDocs.filter(doc => 
    ...
    if (thisWeekDocs.length > 0) {
      groups.push({
        title: 'This Week',
        documents: thisWeekDocs.sort((a, b) => 
          new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime()
        )
      });
    }

    const olderDocs = remainingDocs.filter(doc => 
      !isThisWeek(new Date(doc.updated_at))
    );
    if (olderDocs.length > 0) {
      groups.push({
        title: 'Older',
        documents: olderDocs.sort((a, b) => 
          new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime()
        )
      });
    }
